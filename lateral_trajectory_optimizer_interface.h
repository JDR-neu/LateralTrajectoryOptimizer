...
class LateralTrajectoryOptimizerInterface : public Ipopt::TNLP {
 ...
 private:
  ...
  double * grad_f_ = nullptr;

  void fill_jac_g(const std::size_t nnz_size);

  double* values_jac_g_ = nullptr;

  void fill_hessian(const std::size_t nnz_size);

  double* values_h_lag_ = nullptr;
};
