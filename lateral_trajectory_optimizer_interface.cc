...
LateralTrajectoryOptimizerInterface::~LateralTrajectoryOptimizerInterface() {
  ...
  if (values_jac_g_ != nullptr) {
    delete [] values_jac_g_;
  }

  if (values_h_lag_ != nullptr) {
    delete [] values_h_lag_;
  }
}
...
bool LateralTrajectoryOptimizerInterface::get_nlp_info(int& n, int& m,
  ...
  fill_jac_g(nnz_jac_g);

  fill_hessian(nnz_h_lag);

  grad_f_ = new double[n];

  return true;
}

void LateralTrajectoryOptimizerInterface::fill_jac_g(const std::size_t nnz_size) {
  values_jac_g_ = new double[nnz_size];

  std::fill(values_jac_g_, values_jac_g_ + nnz_size, 0.0);
  ...
  for (std::size_t variable_index = 0; variable_index + 1 < num_of_points_;
    ...
    ++nz_index;
    // d_i''
    values_jac_g_[nz_index] = -0.5 * delta_s_;
    ++nz_index;
    // d_i+1''
    values_jac_g_[nz_index] = -0.5 * delta_s_;
    ++nz_index;
  ...
  for (std::size_t variable_index = 0; variable_index + 1 < num_of_points_;
    ...
    ++nz_index;
    // d_i'
    values_jac_g_[nz_index] = -delta_s_;
    ++nz_index;
    // d_i''
    values_jac_g_[nz_index] = -delta_s_ * delta_s_ / 3.0;
    ++nz_index;
    // d_i+1''
    values_jac_g_[nz_index] = -delta_s_ * delta_s_ / 6.0;
    ++nz_index;
  }
...
void LateralTrajectoryOptimizerInterface::fill_hessian(const std::size_t nnz_size) {
  values_h_lag_ = new double[nnz_size];
...
bool LateralTrajectoryOptimizerInterface::get_starting_point(int n, bool init_x,
  ...
  assert(num_of_variables_ == static_cast<std::size_t>(n));
  assert(init_x == true);
  assert(init_z == false);
...
//  if (new_x || first_time) {
  if (true) {
    ...
    std::memcpy(grad_f_, grad_f, sizeof(double) * n);
  } else {
    std::memcpy(grad_f, grad_f_, sizeof(double) * n);
  }
  return true;
}
...
bool LateralTrajectoryOptimizerInterface::eval_g(int n, const double* x,
  ...
  for (std::size_t i = 0; i + 1 < num_of_points_; ++i) {
    ...
    double end_velocity = v0 + a0 * delta_s_ + 0.5 * j * delta_s_ * delta_s_;
    double end_position = p0 + v0 * delta_s_ + 0.5 * a0 * delta_s_ * delta_s_
        + j * delta_s_ * delta_s_ * delta_s_ / 6.0;

    g[num_of_points_ - 1 + i] = v1 - end_velocity;
    g[2 * (num_of_points_ - 1) + i] = p1 - end_position;
  }
...
bool LateralTrajectoryOptimizerInterface::eval_jac_g(int n, const double* x,
  ...
  if (values == NULL) {
    ...
    assert(nz_index == static_cast<std::size_t>(nele_jac));
    assert(constraint_index == static_cast<std::size_t>(m));
  } else {
    std::memcpy(values, values_jac_g_, sizeof(double) * nele_jac);
    /**
...
bool LateralTrajectoryOptimizerInterface::eval_h(int n, const double* x,
  ...
  assert(num_of_variables_ == static_cast<std::size_t>(nele_hess));
  if (values == nullptr) {
    ...
    std::memcpy(values, values_h_lag_, sizeof(double) * nele_hess);
